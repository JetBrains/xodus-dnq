...
class MigrateDataTest {
    ...
    fun `copy properties`() {
        ...
        youTrackDB.store.executeInTransaction { tx ->
            (tx as YTDBStoreTransaction).assertOrientContainsAllTheEntities(entities)
        }
    }
    ...
    fun `copy an empty entity`() {
        ...
        // do we require typeIds and localIds to match between Xodus and YouTrackDB?
        youTrackDB.store.executeInTransaction {
            youTrackDB.store.getEntity(
                PersistentEntityId(entityId.typeId, entityId.localId)
            )
        }
    }
...
fun Vertex.requireLocalEntityId(): Long =
    getProperty<Long>(LOCAL_ENTITY_ID_PROPERTY_NAME)
