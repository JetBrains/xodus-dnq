...
/**
 * Copied from Xodus release-3.1 branch with small changes to adapt for the YouTrackDB backend.
 */
class TransactionsIsolation2Test : DBTest() {
    ...
    @Ignore("Requires YouTrackDB snapshot isolation (YTDB-510)")
    @Test
    fun test1() {
        transactional { txn ->
        ...
        assertWithMessage("number of cycles where a partial rename was visible (dirty reads)")
            .that(dirtyReads)
            .isEqualTo(0)
    }
