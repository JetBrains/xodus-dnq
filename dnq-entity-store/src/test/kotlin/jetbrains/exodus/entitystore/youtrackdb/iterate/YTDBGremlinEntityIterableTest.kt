...
class YTDBGremlinEntityIterableTest : OTestMixin {
    ...
    fun `unwrapped link read declares the local id order`() {
        ...
        withStoreTx {
            ...
            checkGremlinPattern(
                unwrapped,
                """g.V({rid}).hasLabel("Board").out("HasIssue_link").order().by("localEntityId",Order.asc)"""
            )
        }
    }
    ...
    fun `iterable skip 1`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                issues as YTDBEntityIterable,
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc).skip(1L)"
            )
    ...
    fun `iterable take 2`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                issues as YTDBEntityIterable,
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc).limit(2L)"
            )
    ...
    fun `iterable skip 1 and take 2`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                issues as YTDBEntityIterable,
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc).skip(1L).limit(2L)"
            )
            assertNamesExactlyInOrder(issues, "issue2", "issue3")
        }
    }
    ...
    fun `iterable sort and reverse`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                ...
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc).fold().reverse().unfold()"
            )
            checkGremlin(
                reversedTwice as YTDBEntityIterable,
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc).fold().reverse().unfold().fold().reverse().unfold()"
            )
    ...
    fun `sort iterable and get first`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                sortedIssues,
                "g.V().hasLabel(\"Issue\").order().by(\"name\",Order.asc)"
            )
    ...
    fun `sort descending produces Order desc traversal`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                issues,
                """g.V().hasLabel("Issue").order().by("name",Order.desc)"""
            )
    ...
    fun `sortLinked produces order by out-edge property traversal`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(
                issues as YTDBEntityIterable,
                """g.V().hasLabel("Issue").order().by(__.out("OnBoard_link").values("name"),Order.asc)"""
            )
            assertNamesExactlyInOrder(issues, "issue1", "issue2", "issue3")
        }
    }
    ...
    fun `intersect of sort and condition combines efficiently preserving sort`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(result, """g.V().has("name","issue1").has("priority","high").hasLabel("Issue").order().by("name",Order.asc)""")
    ...
    fun `difference of sort and condition combines efficiently preserving sort`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(result, """g.V().and(__.has("priority","high"),__.not(__.has("name","issue2"))).hasLabel("Issue").order().by("name",Order.asc)""")
            assertNamesExactly(result, "issue1")
        }
    }
    ...
    fun `difference of two sorts with different keys strips right sort and preserves left sort`() {
        ...
        withStoreTx { tx ->
            ...
            checkGremlin(result, """g.V().and(__.has("name","issue1"),__.not(__.has("priority","high"))).hasLabel("Issue").order().by("name",Order.asc)""")
            assertNamesExactly(result, "issue1")
        }
    }
