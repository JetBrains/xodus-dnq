...
class GremlinQueryTest {
    ...
    @Test
    fun `Sort by prop ascending produces simple order by`() {
        assertThat(Sort(Sort.ByProp("name"), SortDirection.ASC).toGremlin())
            .isEqualTo("""__.order().by("name",Order.asc)""")
    }

    @Test
    fun `Sort by prop descending produces simple order by`() {
        assertThat(Sort(Sort.ByProp("name"), SortDirection.DESC).toGremlin())
            .isEqualTo("""__.order().by("name",Order.desc)""")
    }
    ...
    fun `Sort by linked prop ascending traverses edge then orders`() {
        assertThat(Sort(Sort.ByLinked("rel", "name"), SortDirection.ASC).toGremlin())
            .isEqualTo("""__.order().by(__.out("rel_link").values("name"),Order.asc)""")
    }
    ...
    fun `adjacent property sorts combine in primary-first order`() {
        ...
        assertThat(result.toGremlin())
            .isEqualTo("""g.V().has("type","A").hasLabel("Issue").order().by("project",Order.desc).by("type",Order.asc)""")
    }
    ...
    fun `adjacent property sorts retain each direction`() {
        ...
        directions.forEach { (innerDirection, outerDirection) ->
            ...
            val expected = """g.V().has("type","A").hasLabel("Issue").order().by("project",Order.${outerDirection.name.lowercase()}).by("type",Order.${innerDirection.name.lowercase()})"""
    ...
    fun `property sort chain is not flattened across a slice`() {
        ...
        assertThat(result.toGremlin())
            .isEqualTo("""g.V().has("type","A").hasLabel("Issue").order().by("type",Order.asc).skip(1L).order().by("project",Order.asc)""")
    }
    ...
    fun `property sort chain is not flattened across reverse`() {
        ...
        assertThat(result.toGremlin())
            .isEqualTo("""g.V().has("type","A").hasLabel("Issue").order().by("type",Order.asc).fold().reverse().unfold().order().by("project",Order.asc)""")
    }
    ...
    fun `complete property sort chain before a slice is combined`() {
        ...
        assertThat(result.toGremlin())
            .isEqualTo("""g.V().has("type","A").hasLabel("Issue").order().by("project",Order.asc).by("type",Order.asc).skip(1L)""")
    }
    ...
    fun `O3 - intersect preserves every clause of a combined left sort`() {
        ...
        assertThat(result.toGremlin())
            .isEqualTo("""g.V().has("name","a").has("name","b").hasLabel("Issue").order().by("project",Order.desc).by("type",Order.asc)""")
    }
