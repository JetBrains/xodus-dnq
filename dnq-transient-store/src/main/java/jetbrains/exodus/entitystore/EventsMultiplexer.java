...
public class EventsMultiplexer implements TransientStoreSessionListener, IEventsMultiplexer {
  ...
  public void addListener(Entity e, IEntityListener listener) {
    ...
    if ((Object) e == null || listener == null) {
      if (logger.isWarnEnabled()) {
        logger.warn("Can't add null listener to null entity");
      }
      return;
    }
  ...
  public void removeListener(Entity e, IEntityListener listener) {
    ...
    if ((Object) e == null || listener == null) {
      if (logger.isWarnEnabled()) {
        logger.warn("Can't remove null listener from null entity");
      }
      return;
    }
    final EventsMultiplexer.FullEntityId id = new EventsMultiplexer.FullEntityId(e.getStore(), e.getId());
  ...
  public void onClose(TransientEntityStore store) {
    if (logger.isInfoEnabled()) {
      logger.info("Finishing EventsMultiplexer job processor for store " + store);
    }
    JobProcessor processor = getAsyncJobProcessor();
    if (processor != null && !(processor.isFinished())) {
      processor.finish();
      if (logger.isInfoEnabled()) {
        logger.info("EventsMultiplexer closed. Jobs count: " + processor.pendingJobs() + "/" + processor.pendingTimedJobs());
      }
    }
  }
