    ...
    private fun addEntityCreator(transientEntity: TransientEntityImpl, creator: EntityCreator) {
        addChange {
            ...
            if (found != null) {
                ...
                upgradeReadonlyTransactionIfNecessary()
                // New entities were already reset to fresh vertices before replay. Keep that
                // vertex because links replayed earlier may already point to it.
                if (!transientEntity.isNew) {
                    // An initially found persistent entity disappeared; create its replacement.
                    val oldId = transientEntity.idOrNull
