...
class YTDBQueryEntityIterator(
    ...
    companion object {
        ...
        fun executeAndCreate(query: YTDBQuery, txn: YTDBStoreTransaction): YTDBQueryEntityIterator {
            ...
            val iterator = resultSet.toEntityIterator(txn.getStore())
            return YTDBQueryEntityIterator(iterator) { resultSet.close() }
        }
    }

    override fun next(): Entity {
        return YTDBQueryTimeoutException.Companion.withTimeoutWrap { source.next() }
    }

    override fun hasNext(): Boolean {
        val hasNext = YTDBQueryTimeoutException.Companion.withTimeoutWrap { source.hasNext() }
        if (!hasNext) dispose()
        return hasNext
    }
